...
//prepare to page the results
	...
	$param = "&search=".$search."&option_selected=".$option_selected."&show=".$show;
...
//get all the devices from the database
	...
	$sql .= "(\n";
	$sql .= "select d2.domain_uuid from v_domains as d2 \n";
	$sql .= "where d.domain_uuid = d2.domain_uuid \n";
	$sql .= ") as domain_uuid, \n";
	$sql .= "(\n";
...
//lookup the lines
	...
	foreach ($directory as $key => $row) {
		...
		$sql .= "device_uuid = '".$row['device_uuid']."' ";
...
//Line 1 - Register Expires
	...
	if (permission_exists('device_all')) {
		if ($_GET['show'] == 'all') {
			...
			echo button::create(['type'=>'button','label'=>$text['button-show_all'],'icon'=>$_SESSION['theme']['button_icon_all'],'link'=>'?option_selected='.$option_selected.'&show=all']);
		}
	}
	...
	if ($_GET['show'] == "all" && permission_exists('device_all')) {
		echo th_order_by('domain_name', $text['label-domain'], $order_by,$order,'','',"option_selected=".$option_selected."&search=".$search."&show=".$show."");
	}
	echo th_order_by('device_mac_address', $text['label-device_mac_address'], $order_by,$order,'','',"option_selected=".$option_selected."&search=".$search."&show=".$show."");
	echo th_order_by('device_label', $text['label-device_label'], $order_by, $order,'','',"option_selected=".$option_selected."&search=".$search."&show=".$show."");
	if (preg_match ('/line_(.)/',$option_selected)) {
			echo th_order_by($option_selected, $text["label-".$option_selected.""], $order_by,$order,'','',"option_selected=".$option_selected."&search=".$search."&show=".$show."");
		}
	echo th_order_by('device_vendor', $text['label-device_vendor'], $order_by, $order,'','',"option_selected=".$option_selected."&search=".$search."&show=".$show."");
	echo th_order_by('device_template', $text['label-device_template'], $order_by, $order,'','',"option_selected=".$option_selected."&search=".$search."&show=".$show."");
	echo th_order_by('device_label', $text['label-device_profile'], $order_by, $order,'','',"option_selected=".$option_selected."&search=".$search."&show=".$show."");
	echo th_order_by('device_enabled', $text['label-device_enabled'], $order_by, $order,'','',"option_selected=".$option_selected."&search=".$search."&show=".$show."");
	echo th_order_by('device_description', $text['label-device_description'], $order_by, $order,'','',"option_selected=".$option_selected."&search=".$search."&show=".$show."");
	echo "</tr>\n";
